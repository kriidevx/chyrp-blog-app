...
import { Archive, Calendar, Clock, Eye, Heart, MessageCircle, Share2, Tag, User, Zap, BookOpen, TrendingUp } from 'lucide-react';
...
const Old: React.FC<OldPostProps> = ({ createdAt, isArchived = false }) => {
  ...
  const isOld = new Date().getTime() - new Date(createdAt).getTime() > 7 * 24 * 60 * 60 * 1000; // 7 days

  if (!isOld && !isArchived) return null;
  ...
  return (
    <div className="flex items-center space-x-2 mb-3">
      ...
      {isOld && (
        <div className="flex items-center space-x-1 px-2 py-1 bg-slate-100 border border-slate-200 rounded-full">
