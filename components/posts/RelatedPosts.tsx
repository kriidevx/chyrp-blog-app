...
const RelatedPosts = memo<RelatedPostsProps>(({ 
  ...
  const fetchRelatedPosts = useCallback(async () => {
    ...
    try {
      ...
      // Strategy 1: Find posts with matching tags (most relevant)
      if (tags.length > 0) {
        const { data: tagPosts, error: tagError } = await supabase
          ...
          .in('post_tags.tags.slug', tags.slice(0, 3)) // Limit to first 3 tags for performance
          .limit(limit)
      ...
      // Strategy 2: If no tag matches, try category match
      ...
      // Strategy 3: Fallback to recent popular posts
      if (posts.length === 0) {
